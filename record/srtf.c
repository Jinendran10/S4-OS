#include<stdio.h>
void main(){
	int i,n,at[10],bt[10],wt[10],ct[10],tt[10],temp[10],p[10],smallest,count=0,time;
	double avg_wt=0,avg_tt=0,end=0;
	printf("Enter the no.of processes:");
	scanf("%d",&n);
	printf("Enter the arrivial time and bt of the process\n");
	for(i=0;i<n;i++){
		printf("P%d ",i+1);
		scanf("%d%d",&at[i],&bt[i]);
		temp[i]=bt[i];
